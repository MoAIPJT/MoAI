...
export const useStudyMembers = (studyId: number) => {
  return useQuery({
    queryKey: studyKeys.members(studyId),
    queryFn: () => studyService.getStudyMembers(studyId),
    staleTime: 60 * 1000, // 60 seconds
    gcTime: 5 * 60 * 1000, // 5 minutes
...
export const useLeaveStudy = (userId: number, studyId: number, hashId?: string) => {
  ...
  return useMutation({
    ...
    onMutate: async () => {
      ...
      if (studyId) {
        await queryClient.cancelQueries({ queryKey: studyKeys.members(studyId) })

        const previousMembers = queryClient.getQueryData<Member[]>(studyKeys.members(studyId))

        // 현재 사용자를 멤버 목록에서 제거하는 로직
        // 실제로는 현재 사용자 정보가 필요하지만, 여기서는 간단히 처리
        if (previousMembers) {
          queryClient.setQueryData(studyKeys.members(studyId),
    ...
    onError: (err, variables, context) => {
      ...
      if (studyId && context?.previousMembers) {
        queryClient.setQueryData(studyKeys.members(studyId), context.previousMembers)
      }
    },
    onSettled: (data, error, variables, context) => {
      ...
      if (studyId) {
        ...
        queryClient.invalidateQueries({ queryKey: studyKeys.members(studyId) })
      }
    },
    onSuccess: () => {

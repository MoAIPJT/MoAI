...
public class CreateScheduleRequestDto {
    ...
    @Schema(description = "시작 시간", example = "2025-08-05T10:00:00", required = true)
    @NotNull(message = "시작 시간은 필수입니다.")
    @Future(message = "시작 시간은 현재 시간 이후여야 합니다.")
    private LocalDateTime startDatetime;

    @Schema(description = "종료 시간", example = "2025-08-05T12:00:00", required = true)
    @NotNull(message = "종료 시간은 필수입니다.")
    @Future(message = "종료 시간은 현재 시간 이후여야 합니다.")
    private LocalDateTime endDatetime;

    @Schema(description = "일정 제목", example = "주간 회의", required = true)
